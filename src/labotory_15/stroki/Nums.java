package labotory_15.stroki;

import labotory_15.stroki.exceptions.EmptyInput;
import labotory_15.stroki.exceptions.InvalidData;
import labotory_15.stroki.exceptions.WordCount;
        ...
        try {
            while (true) {
                ...
                if (data.length() != 10 || data.charAt(2) != '.' || data.charAt(5) != '.') {
                    throw new InvalidData("Неверный формат (нужно ДД.ММ.ГГГГ)");
                }
                int i;
                for (i = 0; i < data.length(); i++) {
                    if (i != 2 && i != 5 && (data.charAt(i) < '0' || data.charAt(i) > '9')) {
                        throw new InvalidData("В дате должны быть только цифры");
                    }
                }
                if (i < data.length()) {
                    continue;
                ...
                if (month < 1 || month > 12 || day < 1 || day > 31) {
                    throw new InvalidData("Недопустимый месяц или день");
                }
                int[] daysInMonth = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
                boolean leapYear = (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0);
                if (month == 2 && leapYear) daysInMonth[1] = 29;
                if (day > daysInMonth[month - 1]) {
                    throw new InvalidData("В этом месяце нет столько дней");
                }
            ...
            switch (choice) {
                ...
                case 4:
                    try {
                        if (strMethods.isOnlyOneWord(str)) {
                            throw new WordCount(2);
                        } else System.out.println(strMethods.addSpacesBetweenWords(str));
                    } catch (WordCount ex) {
                        System.out.println(ex.getMessage());
                    }
                    break;
